      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-on-surface">Bước 4: Thiết lập các hạng vé</h2>
            <p className="text-xs text-on-surface-variant mt-0.5">
              Mỗi hạng vé tương ứng với một khán đài/phân khu. Quy định: <strong>Khu đứng (STANDING)</strong> và{" "}
        ...
        <div className="space-y-4">
          {ticketTiers.map((tier, idx) => (
            ...
            >
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-primary">Hạng vé / Khán đài #{idx + 1}</span>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-5 gap-3 text-xs">
                <div className="sm:col-span-2">
                  <label className="block font-semibold mb-1">Tên hạng vé / Khán đài</label>
                  <input
                    type="text"
                    value={tier.name}
                    onChange={(e) => {
                ...
                <div>
                  <label className="block font-semibold mb-1">Sức chứa (vé)</label>
                  <input
                    type="number"
                    min={1}
                    value={tier.capacity ?? ""}
                    onChange={(e) => {
                      const newTiers = [...ticketTiers]
                      newTiers[idx].capacity = Math.max(1, Number(e.target.value) || 0)
                      setTicketTiers(newTiers)
                    }}
                    className="w-full px-3 py-2 bg-white border border-outline-variant/60 rounded-xl"
                    placeholder="VD: 500"
